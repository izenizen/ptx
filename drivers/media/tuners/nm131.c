/*
	Driver for Newport Media ISDB-T tuners NMI131, NMI130 and NMI120
...
bool nm131_w(struct i2c_client *c, u16 slvadr, u32 val, u32 sz)
{
	...
	struct i2c_msg msg[] = {
		{.addr = c->addr,	.flags = 0,	.buf = buf,	.len = sz + 4,},
	};
...
bool nm131_r(struct i2c_client *c, u16 slvadr, u8 *dat, u32 sz)
{
	...
	struct i2c_msg msg[] = {
		...
		{.addr = c->addr,		.flags = I2C_M_RD,	.buf = buf,	.len = sz,},
	};

	if (!buf)
		return false;
	ret	= nm131_w(c, slvadr, 0, 0) && i2c_transfer(c->adapter, msg, 2) == 2;
	memcpy(dat, buf, sz);
	kfree(buf);
	return ret;
}
...
int nm131_tune(struct dvb_frontend *fe)
{
	...
	if (!(nm131_w8(c, 1, 0x50)	&&
		nm131_w8(c, 0x47, 0x30)	&&
		nm131_w8(c, 0x25, 0)	&&
		nm131_w8(c, 0x20, 0)	&&
		nm131_w8(c, 0x23, 0x4D)))
		return -EIO;
	while (1) {
		rf = clk_off_f;
		val = rf > 120400000 ? 0x85 : 5;
		if (rf_reg_0x05 != val) {
			nm131_w(c, 0x05, val, 1);
			rf_reg_0x05 = val;
		}
		lofreq = rf;
		v11 = &lo_freq_lut[6];
		...
		nm131_w(c, 8, vhf_filter_cutoff_codes[i].val8_0x08, 1);
		nm131_w(c, 9, vhf_filter_cutoff_codes[i].val8_0x09, 1);
		v14 = lofreq / 1000 * 8 * v45[val];
		nm131_r(c, 0x21, &v15, 1);
		v15 &= 3;
	...
	if (v14 < 16)
		v14 = 16;
	nm131_w(c, 1, (u16)v32 >> 1, 1);
	nm131_w(c, 2, (v32 & 1) | 2 * xo, 1);
	nm131_w(c, 3, xo >> 7, 1);
	nm131_w(c, 4, (xo >> 15) | 16 * v14, 1);
	nm131_r(c, 0x1D, &v15, 1);
	nm131_w(c, 0x1D, 32 * val | (v15 & 0x1F), 1);
	if (lofreq < 300000000) {
		nm131_w(c, 0x25, 0x78, 1);
		nm131_w(c, 0x27, 0x7F, 1);
		nm131_w(c, 0x29, 0x7F, 1);
		nm131_w(c, 0x2E, 0x12, 1);
	} else {
		nm131_w(c, 0x25, 0xF4, 1);
		nm131_w(c, 0x27, 0xEF, 1);
		nm131_w(c, 0x29, 0x4F, 1);
		nm131_w(c, 0x2E, 0x34, 1);
	}
	nm131_w(c, 0x36, lofreq < 150000000 ? 0x54 : 0x7C, 1);
	nm131_w(c, 0x37, lofreq < 155000000 ? 0x84 : lofreq < 300000000 ? 0x9C : 0x84, 1);
	clk_off_f = (clk_off_f << 9) / v14 - 110592000;

	nm131_w(c, 0x164, tune_rf < 300000000 ? 0x600 : 0x500, 4);
	rf = clk_off_f / 6750 + 16384;
	nm131_w(c, 0x230, (adec_ddfs_fq << 15) / rf | 0x80000, 4);
	nm131_w(c, 0x250, ACI_audio_lut, 4);
	nm131_w(c, 0x27C, 0x1010, 4);
	nm131_w(c, 0x1BC, (ddfs_lut << 14) / rf, 4);
	nm131_r(c, 0x21C, (u8 *)(&v32), 4);
	nm131_w(c, 0x21C, (((v32 & 0xFFC00000) | 524288000 / ((clk_off_f >> 14) + 6750)) & 0xC7BFFFFE) | 0x8000000, 4);
	nm131_r(c, 0x234, (u8 *)(&v32), 4);
	nm131_w(c, 0x234, v32 & 0xCFC00000, 4);
	nm131_w(c, 0x210, ((864 * (clk_off_f >> 5) - 1308983296) / 216000 & 0xFFFFFFF0) | 3, 4);
	nm131_r(c, 0x104, (u8 *)(&v32), 4);
	...
	nm131_w(c, 0x104, v32, 4);
	v32 = (v32 & 0xFFFFFFEF) | 0x20000020;
	nm131_w(c, 0x104, v32, 4);
	nm131_r(c, 0x328, (u8 *)(&v14), 4);
	if (v14) {
		v32 &= 0xFFFFFFDF;
		nm131_w(c, 0x104, v32, 4);
		nm131_w(c, 0x104, v32 | 0x20, 4);
	}
	return	nm131_w8(c, 0x23, 0x4C)	&&
		nm131_w8(c, 1, 0x50)	&&
		nm131_w8(c, 0x71, 1)	&&
		nm131_w8(c, 0x72, 0x24)	?
		0 : -EIO;
}
...
int nm131_probe(struct i2c_client *c)
{
	...
	if (nm131_w8(c, 0xB0, 0xA0)	&&
		nm131_w8(c, 0xB2, 0x3D)	&&
		nm131_w8(c, 0xB3, 0x25)	&&
		nm131_w8(c, 0xB4, 0x8B)	&&
		nm131_w8(c, 0xB5, 0x4B)	&&
		nm131_w8(c, 0xB6, 0x3F)	&&
		nm131_w8(c, 0xB7, 0xFF)	&&
		nm131_w8(c, 0xB8, 0xC0)	&&
		nm131_w8(c, 3, 0)	&&
		nm131_w8(c, 0x1D, 0)	&&
		nm131_w8(c, 0x1F, 0)) {
		nm131_w8(c, 0xE, 0x77);
	...
	for (i = 0; i < ARRAY_SIZE(tnr_rf_defaults_lut); i++)
		nm131_w(c, tnr_rf_defaults_lut[i].slvadr, tnr_rf_defaults_lut[i].val, 1);
	nm131_r(c, 0x36, &i, 1);
	nm131_w(c, 0x36, i & 0x7F, 1);	/* no LDO bypass */
	nm131_w(c, tnr_bb_defaults_lut[0].slvadr, tnr_bb_defaults_lut[0].val, 4);
	nm131_w(c, tnr_bb_defaults_lut[1].slvadr, tnr_bb_defaults_lut[1].val, 4);
	for (i = 0; i < ARRAY_SIZE(nm120_rf_defaults_lut); i++)
		nm131_w(c, nm120_rf_defaults_lut[i].slvadr, nm120_rf_defaults_lut[i].val, 1);
	nm131_w(c, 0xA, 0xFB, 1);	/* ltgain */
	return 0;
}
